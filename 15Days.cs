...
Scenario: You are securing communication between two microservices using JWT tokens, but you notice that some requests are failing with "Invalid Token" errors. What could be the most likely cause of this issue?
The tokens are being transmitted over HTTP

The microservices are using different secret keys for token signing

You selected ✅
The tokens are too large to be transmitted

The tokens are being cached

JWT tokens must be signed with a secret key, and if two services use different keys, the signature check will fail, resulting in "Invalid Token" errors. Both services should use the same secret key for validation.

Question 2
Correct Answer
Marks: 1/1

What is the best practice for ensuring fault tolerance in a microservices architecture?
Use a retry mechanism with exponential backoff for failed requests

You selected ✅
Restart the entire system when a service fails

Ensure all services are tightly coupled to reduce failures

Disable timeouts for service requests

Exponential backoff with retries helps handle transient failures in microservices. It allows requests to be retried after progressively longer intervals, reducing load on the system and increasing resilience.

Question 3
Correct Answer
Marks: 1/1

Scenario: You are designing a microservice architecture using ASP.NET Core with Ocelot Gateway. One of the services frequently experiences high latency due to heavy load. What solution would help distribute the load across multiple instances of the service?
Enable JWT authentication in the service

Increase the service’s memory allocation

Enable service auto-scaling in Azure

You selected ✅
Implement rate limiting in Ocelot

Question 4
Correct Answer
Marks: 1/1

Which of the following is the most appropriate best practice for managing sensitive data transmitted between services in a microservices architecture?
Encrypt data at rest but not in transit

Store sensitive data in environment variables

Use HTTP for internal service communication

Implement TLS/SSL for all service-to-service communication
...
What is the primary benefit of using a service discovery mechanism in a microservices architecture?
To ensure that services can find and communicate with each other dynamically

You selected ✅
To improve the performance of monolithic applications

To store configuration data for services

To allow services to cache data locally

Service discovery ensures that microservices can locate each other without manual configuration, enabling scalable and dynamic communication between services.

Question 6
Correct Answer
Marks: 1/1

Which security feature is recommended for preventing Cross-Site Request Forgery (CSRF) attacks in an ASP.NET Core API?
Using JWT tokens with CSRF tokens
...
Allowing all origins in CORS policies

JWT tokens protect authentication, while CSRF tokens prevent unauthorized form submissions by ensuring requests come from trusted sources. Both are critical for securing APIs against CSRF attacks.

Question 7
Correct Answer
Marks: 1/1

What is the primary role of the Ocelot Gateway in a microservices architecture?
To route, load balance, and secure API requests between clients and microservices

You selected ✅
To authenticate users using JWT tokens

To store session state for user data
...
Which of the following is the most important best practice when designing RESTful APIs in ASP.NET Core?
Use session state to manage user authentication

Always include query parameters in all API requests

Store large files within the API responses
...
Which of the following patterns is commonly used to decouple services and handle failures gracefully in a microservices architecture?
Observer Pattern
...
The Circuit Breaker Pattern allows services to fail fast when dependencies are unavailable, preventing repeated failed requests and reducing pressure on downstream services.

Question 10
Correct Answer
Marks: 1/1

Scenario: You are building a REST API for managing orders in your e-commerce platform. You need to ensure the API can handle large volumes of orders across multiple regions while maintaining low latency. Which architecture would be most suitable for this requirement?
Monolithic architecture with centralized SQL database

REST API with role-based authentication

Monolithic architecture with Azure SQL Database

Microservices architecture with Cosmos DB and geo-replication
...
When using Ocelot as an API Gateway in ASP.NET Core, which of the following is a best practice for handling large-scale microservice traffic?
Enable load balancing and routing to multiple microservice instances

You selected ✅
Store all request and response payloads in cookies

Disable rate limiting to ensure all requests are processed

Use session state to persist user information across microservices

Question 12
Correct Answer
Marks: 1/1

Scenario: A microservices-based application has several services deployed in different regions. However, users report slow response times when accessing certain services from specific regions. How can you resolve this issue?
Configure API Gateway to route traffic to the nearest service
...
Enable CSRF protection in the services

Question 13
Correct Answer
Marks: 1/1

Scenario: You are troubleshooting a RESTful API that returns HTTP 500 errors under heavy load. After inspecting the logs, you realize that the API is running out of connections to the database. What could be a potential fix for this issue?
Use JWT tokens to authenticate API requests

Use session state for database access
...
Which architectural style is characterized by loosely coupled services that communicate over the network using lightweight protocols?
...
Which protocol is most commonly used for secure communication between microservices?
HTTP
...
Which of the following is a core principle of RESTful architecture?
Database indexing
...
What does the term "URI" refer to in RESTful APIs?
Uniform Resource Index

Uniform Registration Interface
...
Which of the following tools is commonly used for testing and debugging RESTful APIs?
Visual Studio
...
In Postman, which method would you use to send data to an API for creating a new resource?
PUT
...
Which tool can be used to inspect HTTP traffic, including requests and responses between the client and server?
Postman
...
In Postman, what is used to add custom headers to an API request?
Body
...
What is the primary purpose of the "Console" in Postman?
To create new requests

To view responses to API requests

To store API documentation

To view request logs and error details
...
What is the purpose of URI routing in ASP.NET Core?
To store database connection strings

To define server-side session handling

To map HTTP requests to specific controller actions

You selected ✅
To implement security features in an application

Question 12
Correct Answer
Marks: 1/1

In ASP.NET Core, which attribute is used to define a route for a controller action?
[Route]
...
In ASP.NET Core, what is the correct syntax to define a route parameter in a URI?
/books?id
...
In the context of ASP.NET Core, which HTTP method is used to update an existing resource?
DELETE
...
Which of the following is the correct way to return a 404 status code if a resource is not found in a RESTful API in ASP.NET Core?
Return Ok()
